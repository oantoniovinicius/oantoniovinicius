<div align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=7f3ace&height=120&section=header" />
</div>


[![Versão em Português](https://img.shields.io/badge/🌐_Versão_em_Português-7F3ACE?style=for-the-badge)](README.md)

---
[![Typing SVG](https://readme-typing-svg.demolab.com/?font=Fira+Code&pause=1000&color=F7F5F5&width=435&lines=Hello%2C+my+name+is+Antonio!)](https://git.io/typing-svg)

<div align="center">
  <img height="200" src="https://raw.githubusercontent.com/skstef/skstef/main/assets/cat-coder.gif" />

  **Backend Developer** focused on Java, Spring Boot, and distributed systems.  
  Experience with microservices, hexagonal architecture, and event-driven architecture.
</div>

<div align="center">

[![E-mail](https://img.shields.io/badge/-Email-000?style=for-the-badge&logo=gmail&logoColor=7F3ACE)](mailto:devantoniovinicius@gmail.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-000?style=for-the-badge&logo=linkedin&logoColor=7F3ACE)](https://www.linkedin.com/in/antoniovinicius/)
[![GitHub](https://img.shields.io/badge/-GitHub-000?style=for-the-badge&logo=github&logoColor=7F3ACE)](https://github.com/oantoniovinicius)

</div>

---

<div align="center">

## ⚙️ Tech Stack

| Backend | Databases & Messaging | DevOps & Tools |
| :---: | :---: | :---: |
| [![My Skills](https://skillicons.dev/icons?i=java,spring)](https://skillicons.dev) | [![My Skills](https://skillicons.dev/icons?i=postgresql,rabbitmq)](https://skillicons.dev) | [![My Skills](https://skillicons.dev/icons?i=docker,git,github,maven,postman,idea)](https://skillicons.dev) |

</div>

---

<div align="center">

## 🚀 Projects

</div>

| Project | Description | Tech Stack |
| :--- | :--- | :--- |
| **[RiskGuard](https://github.com/oantoniovinicius/risk-guard)** | Event-driven antifraud platform with 32 endpoints, 7 bounded contexts, hexagonal architecture, and asynchronous messaging with RabbitMQ. | `Java`, `Spring Boot`, `RabbitMQ`, `PostgreSQL`, `Docker` |
| **[Wallet Transfer API](https://github.com/oantoniovinicius/wallet-transfer-api)** | Payment API with fund transfers, transactional control, and integration with external authorization and notification services. | `Java`, `Spring Boot`, `PostgreSQL`, `Docker` |
| **[TaskManager API](https://github.com/oantoniovinicius/taskmanager-api)** | REST API with 12 endpoints, stateless JWT authentication, and role-based access control (RBAC) with Spring Security. | `Java`, `Spring Boot`, `PostgreSQL`, `Spring Security` |
| **[d-Lemma](https://github.com/oantoniovinicius/d-lemma)** | Collaborative discussion platform with 16 endpoints, 3-state invite system, and password recovery with expirable tokens. | `Java`, `Spring Boot`, `PostgreSQL`, `Spring Security` |

---

<div align="center">

## 📊 GitHub Stats

<img height="180em" src="https://github-readme-stats-sigma-five.vercel.app/api?username=oantoniovinicius&show_icons=true&theme=midnight-purple&include_all_commits=true&count_private=true&hide_border=true" />
<img height="180em" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=oantoniovinicius&layout=compact&langs_count=6&theme=midnight-purple&hide_border=true&hide=html,css" />

</div>

<div align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=7f3ace&height=120&section=footer" />
</div>
